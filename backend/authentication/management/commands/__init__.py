    ...
    def handle(self, *args, **options):
        if options['list_locked']:
            qs = User.objects.filter(
                models.Q(account_locked=True) | models.Q(failed_attempts__gte=5)
            ).order_by('username')
            if not qs.exists():
                self.stdout.write(self.style.SUCCESS('No locked users found.'))
                return
            self.stdout.write(f"{'Username':<22}{'Email':<32}{'Locked':>8}{'Attempts':>10}")
        ...
        if not qs.exists():
            self.stdout.write(self.style.WARNING('No users matched the given criteria.'))
            return
